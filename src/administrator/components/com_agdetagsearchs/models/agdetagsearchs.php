 ...
 * @since  1.6
 */
class AgdetagsearchsModelAgdetagsearchs extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function populateState($ordering = 'a.title', $direction = 'asc')
	{
		...
		$this->setState(
			'filter.category_id', $this->getUserStateFromRequest($this->context . '.filter.category_id', 'filter_category_id',
				'',
				'cmd'
			)
		);
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__agdetagsearchs', 'a'));
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					...
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_agdetagsearchs.agdetagsearch')
				);
		}
